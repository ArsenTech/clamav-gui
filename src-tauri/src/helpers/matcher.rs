...
pub static EXCLUSIONS: Lazy<Arc<Mutex<Option<ExclusionMatcher>>>> = Lazy::new(|| Arc::new(Mutex::new(None)));

pub fn apply_exclusions(
     app: &tauri::AppHandle,
     args: &mut Vec<String>,
) -> Result<(), String> {
     use crate::helpers::get_exclusions;
     use crate::helpers::path::path_to_regex;

     let exclusions = get_exclusions(app)?;
