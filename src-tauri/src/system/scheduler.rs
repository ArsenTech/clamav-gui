...
use crate::{
     helpers::{
          resolve_command, scheduler::{get_last_run_time_windows, load_scheduler_file, save_scheduler_file}, silent_command
     },
     types::{
     ...
     let items: Vec<SchedulerEvent> = scheduler_file
          ...
          .map(|entry| {
               let last_run = match entry.last_run {
                    Some(time) => {
                         get_last_run_time_windows(&entry.id).unwrap_or(Some(time))
                    }
